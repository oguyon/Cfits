...
int GPU_loop_MultMat_setup(int index, char *IDcontrM_name, char *IDwfsim_name, char *IDoutdmmodes_name, long NBGPUs, int *GPUdevice, int orientation, int USEsem, int initWFSref, long loopnb)
{
    ...
    if(gpumatmultconf[index].init == 0)
    {
        ...
        for(device = 0; device < gpumatmultconf[index].NBstreams; device++)
        {
            ...
            sprintf(sname, "i%02d_gpu%02d_sem1", index, GPUdevice[device]);
            ...
            sprintf(sname, "i%02d_gpu%02d_sem2", index, GPUdevice[device]);
            ...
            sprintf(sname, "i%02d_gpu%02d_sem3", index, GPUdevice[device]);
            ...
            sprintf(sname, "i%02d_gpu%02d_sem4", index, GPUdevice[device]);
            ...
            sprintf(sname, "i%02d_gpu%02d_sem5", index, GPUdevice[device]);
