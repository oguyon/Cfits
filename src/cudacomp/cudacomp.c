...
int CUDACOMP_extractModesLoop(char *DMact_stream, char *DMmodes, char *DMmodes_gain, char *DMmodes_val, int GPUindex, int FILTERMODES, char *IDfiltmult_name, char *IDoutfilt_name)
{
    ...
    if(FILTERMODES==1)
        {
            ...
            imOK = 1;
            if(ID_modeval_mult == -1)
                imOK = 0;
            else
                {
                    if(data.image[ID_modeval_mult].md[0].size[0]!=NBmodes)
                        {
                            imOK = 0;
                            delete_image_ID(IDfiltmult_name);
                        }
                }
            if(imOK==0)
            {
            ...
            for(k=0;k<NBmodes;k++)
                {
                    if(k<2)
                        data.image[ID_modeval_mult].array.F[k] = 0.99;
                    else
                        data.image[ID_modeval_mult].array.F[k] = 0.99;
                }
   ...
   if (sigaction(SIGSEGV, &data.sigact, NULL) == -1) {
    ...
    if(TRACEMODE==1)
        {
            sizearraytmp = (long*) malloc(sizeof(long)*2);
            sprintf(traceim_name, "%s_trace", DMmodes_val);
