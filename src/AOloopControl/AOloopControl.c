...
int AOloopControl_GPUmodecoeffs2dm_filt_loop(char *modecoeffs_name, char *DMmodes_name, int semTrigg, char *out_name, int GPUindex, long loop, int offloadMode)
{
	...
	#ifdef HAVE_CUDA
    GPU_loop_MultMat_setup(1, DMmodes_name, imnamecorr, out_name, GPUcnt, GPUsetM, orientation, use_sem, initWFSref, 0);        
	...
	while(1==1)
        {	
			...
			GPU_loop_MultMat_execute(1, &status, &GPUstatus[0], alpha, beta, write_timing);
