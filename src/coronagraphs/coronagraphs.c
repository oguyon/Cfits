...
int telescope_pupil_Subaru_inside1(double x, double y)
{
  int inside;
  double r,PA;
...
int coronagraphs_make_SUBARU_pupil()
{
  ...
  for(ii=0;ii<Ssize;ii++)
	...
	*/
	...
	//x = r0*cos(PA);
	//y = r0*sin(PA);
	data.image[ID].array.F[jj*Ssize+ii] = telescope_pupil_Subaru_inside1(x,y);
      }
...
int CORONAGRAPHS_PIAAperturbation(double *zarray, long *zindex, long NBzern, double ratio)
{
  double trad_pix = CORONAGRAPHS_TDIAM/CORONAGRAPHS_PSCALE/2.0;
...
int coronagraph_userfunc()
{
  ...
  zindex[0] = 12; //24;
  zarray[0] = 1.0e-5;
  CORONAGRAPHS_PIAAperturbation(zarray, zindex, NBzern, ratio);
  free(zarray);
  free(zindex);
  exit(0);
